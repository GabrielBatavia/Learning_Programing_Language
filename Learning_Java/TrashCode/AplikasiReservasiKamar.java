...
public class AplikasiReservasiKamar {
    ...
    private static ArrayList<String> registerednamaPenggunas = new ArrayList<>();
    ...
    private static int authenticateUser(ArrayList<String> namaPenggunas, ArrayList<String> passwords, String namaPenggunaInput, String passwordInput) {
        if (namaPenggunas.contains(namaPenggunaInput)) {
            int userIndex = namaPenggunas.indexOf(namaPenggunaInput);
            if (passwords.get(userIndex).equals(passwordInput)) {
                System.out.println("Selamat datang, " + namaPenggunaInput + "!");
                return userIndex;
            }
        }
        System.out.println(ANSI_RED + "namaPengguna atau sandi salah. Silakan coba lagi." + ANSI_RESET);
        return -1;
    }
    ...
    private static void registerUser(ArrayList<String> namaPenggunas, ArrayList<String> passwords, ArrayList<User> users, Scanner input) {
        System.out.println("════════════════════════════════════════════════════════");
        ...
        String newnamaPengguna = input.nextLine();
        
        if (!namaPenggunas.contains(newnamaPengguna)) {
            System.out.print("Masukkan Password baru      : ");
            String newPassword = input.nextLine();
        
            // Enkripsi atau hash sandi dan NIK di sini
    
            namaPenggunas.add(newnamaPengguna);
            passwords.add(newPassword);
    
            users.add(new User(newnamaPengguna, newPassword, namaLengkap, alamat, tanggalLahir, nik)); // Tambahkan user baru dengan informasi lengkap
    ...
    private static void makeBooking(int[] stockRooms, String[][] bookings, int userIndex, Scanner input) {
        ...
        System.out.println("Pilihan Tipe Kamar dan Ketersediaan:");
        ...
        hariReservasi = input.next();
        if (isValidDate(hariReservasi) && !unavailableDates.contains(hariReservasi)) {
         ...
         bookings[userIndex][1] = hariReservasi;
    ...
    public static void viewRegisteredUsers(ArrayList<User> users) {
        if (users.isEmpty()) {
            System.out.println(ANSI_RED + "Tidak ada pengguna yang terdaftar."+ANSI_RESET);
            return;
        }
    
        System.out.println(ANSI_RED + "Daftar Nama Pengguna Terdaftar:"+ ANSI_RESET);
        for (User user : users) {
            System.out.println(user.namaPengguna);
        }
    }
    ...
    public static void viewUserDetails(ArrayList<User> users, Scanner input) {
        ...
        System.out.println(ANSI_RED + "namaPengguna tidak ditemukan." + ANSI_RESET);
    }

    public static void deleteUser(ArrayList<User> users, ArrayList<String> namaPenggunas, Scanner input) {
        System.out.print("Masukkan namaPengguna pengguna yang akan dihapus: ");
        String namaPengguna = input.nextLine();
    
        for (int i = 0; i < users.size(); i++) {
            if (users.get(i).namaPengguna.equals(namaPengguna)) {
                users.remove(i);
                namaPenggunas.remove(namaPengguna);
    ...
    while (isProgramRunning) {
        if (currentUser == null && !isAdminLoggedIn) {
            ...
            switch (choice) {
                ...
                String namaPenggunaInput = input.nextLine(); 
                System.out.print("Masukkan Password     : ");
                String passwordInput = input.nextLine(); 

                    // Login
                    if (namaPenggunaInput.equals(ADMIN_namaPengguna) && passwordInput.equals(ADMIN_PASSWORD)) {
                        // Admin login
                        isAdminLoggedIn = true;
                        System.out.println(ANSI_GREEN + "Login Sukses." + ANSI_RESET);
                    } else {
                        // User login
                        userIndex = authenticateUser(registerednamaPenggunas, registeredPasswords, namaPenggunaInput, passwordInput);
                        if (userIndex != -1) {
                ...
                case 2:
                    // Daftar (Register)
                    registerUser(registerednamaPenggunas, registeredPasswords, users, input);
                    break;
                case 3:
            ...
            switch (userChoice) {
                ...
                if (statusCheckIn.equals("belum")) {
                    ...
                    System.out.println(ANSI_PURPLE + "═══════════════════════════════════════════════" + ANSI_RESET);
                    System.out.println(ANSI_CYAN + "namaPengguna            : " + ANSI_RESET + registerednamaPenggunas.get(userIndex) + ANSI_RESET);
            ...
            if (bookings[userIndex] != null && statusPemesanan.equals("Dipesan")) {
                if (statusCheckIn.equals(STATUS_APPROVED_CHECKIN)) {
                    ...
                    System.out.println(ANSI_GREEN + "Permintaan Check-Out Anda sudah disetujui oleh admin." + ANSI_RESET);
                    System.out.println();
                    ...
                    System.out.print("Beri nilai (1-5) : ");
                    int nilai = input.nextInt();
                    input.nextLine(); // Membersihkan buffer input
            
                    if (nilai >= 1 && nilai <= 5) {
                        bookings[userIndex][3] = "Rating : " + nilai;
            ...
            switch (adminChoice) {
                case 1:
                    ...
                    for (int i = 0; i < bookings.length; i++) {
                        if (bookings[i][0] != null) {
                            System.out.println("namaPengguna    : " + registerednamaPenggunas.get(i));
                ...
                case 2:
                    ...
                    System.out.println("Menyetujui Check in :");
                    for (int i = 0; i < bookings.length; i++) {
                        if (bookings[i] != null && bookings[i][8] != null && bookings[i][8].equals(STATUS_PENDING_CHECKIN)) {
                            System.out.println("Check in ditunda untuk pengguna : " + registerednamaPenggunas.get(i));
                    ...
                    //boolean userFound = false;
                    for (int i = 0; i < registerednamaPenggunas.size(); i++) {
                        if (registerednamaPenggunas.get(i).equals(namaPenggunaCheckIn) && bookings[i] != null && bookings[i][8] != null && bookings[i][8].equals(STATUS_PENDING_CHECKIN)) {
                            bookings[i][8] = STATUS_APPROVED_CHECKIN;
                            System.out.println( namaPenggunaCheckIn + " : Check in disetujui");
                            userFound = true;
                            break;
                        }
                    }
                    if (!userFound) {
                        System.out.println("No pending check-in found for the namaPengguna : " + namaPenggunaCheckIn);
                    }
                    break;
                ...
                userFound = false;
                for (int i = 0; i < registerednamaPenggunas.size(); i++) {
                    if (registerednamaPenggunas.get(i).equals(namaPenggunaCheckOut) && bookings[i] != null && bookings[i][8].equals(STATUS_PENDING_CHECKOUT)) {
                ...
                if (!userFound) {
                    System.out.println("No pending check-out found for the namaPengguna : " + namaPenggunaCheckOut);
                }
                break;
                ...
                case 7:
                    deleteUser(users, registerednamaPenggunas, input);
                break;
